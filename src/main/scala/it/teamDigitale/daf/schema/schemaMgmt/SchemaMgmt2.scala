...
class SchemaMgmt2(convSchemaIn: ConvSchema )  extends Serializable with Logging{

  /*
  
  case class FinalSchema (
      name: String,  //delete
      nameDataset: String,  //delete
      cat: Seq[String],  //delete
      groupOwn: String,  //delete
      owner: String,  //delete
      src: Map[String, String],  //delete
      stdSchemaName: Option[String],//delete
      fields: Option[JsArray],  //Put as JsArray for now  //delete
  ...
  def getFinalSchema(): FinalSchema = {
    ...
    FinalSchema (
      ...
      owner = convSchema.owner,
      src = Map(),
      stdSchemaName = Option(""),
      fields = Option(JsArray()),
      reqStdFields = List(),
      optStdFields = List() 
    )
  }
